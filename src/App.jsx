...
const FAMILY_RULES = [
  {
    family: "Labor Flexibility / Shift Marketplace",
    keywords: ["open shift", "shift marketplace", "shift swaps", "shift fill", "labor flexibility", "scheduling", "agency labor", "gig", "coverage"]
  },
  {
    family: "Safety / Compliance",
    keywords: ["safety", "compliance", "recall", "red alert", "food safety", "risk", "regulatory", "agreement"]
  },
  {
    family: "Learning / Training",
    keywords: ["learning", "training", "lms", "scorm", "preboarding", "pos training", "micro-learning"]
  },
  {
    family: "Adoption / Daily Hub",
    keywords: ["adoption", "daily hub", "daily", "frontline hub", "digital workplace", "single pane", "one app", "stickiness", "dau"]
  },
  {
    family: "Engagement → Execution",
    keywords: ["engagement", "execution", "communications", "channels", "comments", "sentiment", "voice", "collaboration"]
  },
  {
    family: "Manager Productivity",
    keywords: ["manager", "hours", "admin", "approvals", "labor savings", "productivity"]
  },
  {
    family: "Workflow / Systems / Orchestration",
    keywords: ["orchestration", "task connect", "integration", "erp", "qualtrics", "ukg", "dailypay", "shadow it", "systems", "tool sprawl", "governance"]
  },
  {
    family: "Customer Experience / Feedback",
    keywords: ["customer", "cx", "feedback", "survey", "remediation", "cleanliness"]
  },
  {
    family: "Analytics / AI Opportunity",
    keywords: ["analytics", "ai", "pattern", "prioritization", "segmentation", "coaching map", "root cause"]
  }
];
...
function personaMatches(story, personaId) {
  ...
  const text = storyText(story);
  return includesAny(text, persona.keywords);
}
...
function findRelatedStories(selectedStory, allStories) {
  ...
  return allStories
    ...
    .map((story) => {
      ...
      const sharedPain = [...(story.primaryPains || []), ...(story.secondaryPains || [])].filter((p) =>
        [...(selectedStory.primaryPains || []), ...(selectedStory.secondaryPains || [])].includes(p)
      );
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen w-full font-sans antialiased transition-colors duration-200 ${darkMode ? "bg-[#121212] text-neutral-100" : "bg-[#F2F2F7] text-neutral-950"}`}>
      <header className={`px-5 2xl:px-8 py-3 flex items-center justify-between border-b sticky top-0 z-40 ${darkMode ? "bg-[#1C1C1E] border-neutral-800" : "bg-white border-neutral-200"}`}>
        <div className="flex items-center gap-3">
          <Folder size={16} className="text-blue-500" />
          <div>
            <div className="text-xs font-semibold tracking-tight text-neutral-500 dark:text-neutral-300 font-mono">WORKJAM STORY SELECTOR</div>
            <div className="text-[10px] text-neutral-400">Dynamic presales story matching · {STORIES_DATA.length} stories</div>
          </div>
        </div>
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-12 xl:grid-cols-14 min-h-[calc(100vh-58px)]">
        <aside className={`md:col-span-3 xl:col-span-3 border-r p-4 2xl:p-5 flex flex-col gap-5 ${darkMode ? "bg-[#1C1C1E] border-neutral-800" : "bg-[#F2F2F7] border-neutral-300"} ${mobileView !== "sidebar" ? "hidden md:flex" : "flex"}`}>
          <div>
        ...
        <section className={`md:col-span-4 xl:col-span-4 border-r flex flex-col ${darkMode ? "bg-[#18181B] border-neutral-800" : "bg-white border-neutral-200"} ${mobileView === "sidebar" || (mobileView === "detail" && selectedStory) ? "hidden md:flex" : "flex"}`}>
          <div className="p-3 2xl:p-4 border-b border-neutral-200 dark:border-neutral-800 flex flex-col gap-2 shrink-0">
          ...
          {topStories.length > 0 && activeFolder === "recommend" && (
            <div className="p-3 2xl:p-4 border-b border-neutral-200 dark:border-neutral-800">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-neutral-100 dark:divide-neutral-800">
            ...
            ) : filteredStories.map((story) => <StoryRow key={story.id} story={story} selected={selectedStory?.id === story.id} darkMode={darkMode} onClick={() => { setSelectedStory(story); setMobileView("detail"); }} />)}
          </div>
        </section>

        <main className={`md:col-span-5 xl:col-span-7 p-5 2xl:p-8 flex flex-col overflow-y-auto max-h-[calc(100vh-58px)] ${darkMode ? "bg-[#121212]" : "bg-white"} ${mobileView !== "detail" ? "hidden md:flex" : "flex"}`}>
          {selectedStory ? (
            <div className="flex flex-col gap-6 text-left w-full px-0 lg:px-4 2xl:px-8">
              ...
              <div className="border-b border-neutral-100 dark:border-neutral-800 pb-4">
                ...
                <div className="flex flex-wrap gap-1.5 mt-3"><Badge label={`Score ${selectedStory.matchScore ?? getStoryScore(selectedStory, filters)}`} /><Badge label={`${selectedStory.confidenceScore}% confidence`} /><Badge label={selectedStory.proofLevel} tone={selectedStory.proofLevel} /><Badge label={selectedStory.proofTheme} /></div>
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <DetailCard title="Why This Matched" icon={Sparkles}>
                  {selectedStory.matchReasons?.length ? <ul className="space-y-1.5">{selectedStory.matchReasons.map((reason, index) => <li key={index} className="flex gap-2"><span className="text-emerald-500">✓</span><span>{reason}</span></li>)}</ul> : <p>This story is currently selected from the full story set.</p>}
                </DetailCard>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-5 gap-4 items-start">
                <div className="xl:col-span-3">
                  ...
                  <p className="text-sm 2xl:text-[15px] text-neutral-700 dark:text-neutral-300 leading-relaxed">{selectedStory.narrative}</p>
                </div>
                <div className="xl:col-span-2"><DetailCard title="Profile" icon={Layers}><p>{selectedStory.profile}</p></DetailCard></div>
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <div className={`p-4 rounded-xl border ${darkMode ? "bg-emerald-500/10 border-emerald-500/30" : "bg-emerald-50 border-emerald-200"}`}>
                  <h4 className="text-[10px] text-emerald-600 dark:text-emerald-300 font-extrabold uppercase tracking-widest font-mono mb-2.5 flex items-center gap-1.5"><TrendingUp size={12} /><span>Key Outcomes / Proof</span></h4>
                  <ul className="flex flex-col gap-2">{selectedStory.outcomes?.map((outcome, index) => <li key={index} className="text-xs text-neutral-800 dark:text-neutral-200 flex items-start gap-2"><span className="text-emerald-500 font-bold mt-0.5">•</span><span className="font-medium">{outcome}</span></li>)}</ul>
                </div>
                <div className={`p-4 rounded-xl border ${darkMode ? "bg-blue-500/10 border-blue-500/30" : "bg-blue-50 border-blue-100"}`}>
                  <h4 className="text-[10px] text-blue-500 font-extrabold uppercase tracking-widest font-mono mb-2 flex items-center gap-1"><Target size={12} /><span>Best Demo Angle</span></h4>
                  <p className="text-sm text-neutral-800 dark:text-neutral-200 font-semibold leading-relaxed">{selectedStory.demoAngle}</p>
                  <div className="mt-4 border-t border-blue-500/10 pt-3"><p className="text-[10px] uppercase tracking-widest font-mono text-blue-500/70 mb-1">Why it matters</p><p className="text-xs text-neutral-600 dark:text-neutral-300 leading-relaxed">{selectedStory.whyItMatters}</p></div>
                </div>
              </div>

              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <DetailCard title="Modules & Trigger Phrases" icon={MessageSquare}>
                  <div className="mb-3"><p className="text-[10px] uppercase tracking-widest font-mono text-neutral-400 mb-1">Modules</p><PillList items={selectedStory.modules} /></div>
                  <div><p className="text-[10px] uppercase tracking-widest font-mono text-neutral-400 mb-1">Trigger phrases</p><PillList items={(selectedStory.triggerPhrases || []).slice(0, 10)} /></div>
                </DetailCard>
              ...
              <div className={`p-4 rounded-xl border ${darkMode ? "bg-neutral-900 border-neutral-800" : "bg-neutral-50 border-neutral-200 shadow-sm"}`}>
                <div className="flex items-center justify-between border-b pb-2.5 mb-3 border-neutral-200 dark:border-neutral-800"><span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest font-mono flex items-center gap-1.5"><Wand2 size={12} />Copy Builder</span><div className="flex gap-1 flex-wrap justify-end"><ModeButton mode="talkTrack" label="Talk" active={copyMode} setActive={setCopyMode} /><ModeButton mode="discovery" label="Questions" active={copyMode} setActive={setCopyMode} /><ModeButton mode="proof" label="Proof" active={copyMode} setActive={setCopyMode} /><ModeButton mode="demo" label="Demo" active={copyMode} setActive={setCopyMode} /></div></div>
                <div className="p-3 bg-white dark:bg-[#121212] border border-neutral-200 dark:border-neutral-800 rounded-lg max-h-[260px] overflow-y-auto font-mono text-[11px] leading-relaxed text-neutral-600 dark:text-neutral-300 whitespace-pre-wrap">{generatedCopy}</div>
                <button onClick={() => handleCopy(generatedCopy, "copy-builder")} className={`w-full py-1.5 rounded-lg text-xs font-semibold mt-3 flex items-center justify-center gap-1.5 transition-all ${copiedId === "copy-builder" ? "bg-emerald-500 text-white" : "bg-blue-500 hover:bg-blue-600 text-white shadow-sm"}`}>{copiedId === "copy-builder" ? <Check size={12} /> : <Copy size={12} />}<span>{copiedId === "copy-builder" ? "Copied!" : "Copy Active Draft"}</span></button>
              </div>
            </div>
          ) : <div className="p-12 text-center text-neutral-400 flex flex-col items-center justify-center h-full max-w-sm mx-auto"><FileText className="text-neutral-300 dark:text-neutral-700 mb-2" size={32} /><p className="text-xs">No story matches the current filters.</p></div>}
        </main>
      </div>
    </div>
  );
...
function FilterSection({ title, children }) {
  return <div className="pt-2 border-t border-neutral-200 dark:border-neutral-800"><h3 className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest px-2 mb-2">{title}</h3>{children}</div>;
}
...
function LabeledSelect({ label, value, onChange, options }) {
  return <label className="flex flex-col gap-1"><span className="text-[9px] uppercase tracking-widest font-mono font-bold text-neutral-400">{label}</span><Select value={value} onChange={onChange} options={options} /></label>;
}
...
function PillList({ items = [] }) {
  ...
  return <div className="flex flex-wrap gap-1">{items.map((item) => <span key={item} className="px-2 py-0.5 rounded-md bg-white dark:bg-neutral-800 text-neutral-500 dark:text-neutral-300 text-[10px] font-semibold border border-neutral-200 dark:border-neutral-700">{item}</span>)}</div>;
}

function ModeButton({ mode, label, active, setActive }) {
  return <button onClick={() => setActive(mode)} className={`px-2 py-0.5 text-[10px] rounded font-semibold transition-colors ${active === mode ? "bg-blue-500 text-white" : "text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300"}`}>{label}</button>;
}
